...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, args, text_or_image=None):
        ...
        self.transformer = Transformer(width, layers, heads, adapter_flag=True, args=args, text_or_image=text_or_image)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(  # image
                input_resolution=image_resolution,
    ...
    def encode_image(self, image, val_image_task_id=None):
        if val_image_task_id != None:
            global val_task_id
            val_task_id = val_image_task_id
        return self.visual(image.type(self.dtype))

    def encode_text(self, text, val_text_task_id=None):
        if val_text_task_id != None:
            global val_task_id
            val_task_id = val_text_task_id
